...
if(isset($_POST['submit'])){
   ...
   if($check_cart->rowCount() > 0){
      ...
      if($address == ''){
         ...
         $insert_order = $conn->prepare("INSERT INTO `orders`(user_id, first_name, number, email, method, address, total_products, total_price) VALUES(?,?,?,?,?,?,?,?)");
         $insert_order->execute([$user_id, $first_name, $number, $email, $method, $address, $total_products, $total_price]);
...
<form action="" method="post">
   ...
   <div class="user-info">
      <h3>your info</h3>
      <p><i class="fas fa-user"></i><span><?= $fetch_profile['first_name'] ?><?php echo ' '; ?><?= $fetch_profile['last_name'] ?></span></p>
